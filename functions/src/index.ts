...
const REQUEST_TYPE_NAMES: { [key: string]: string } = {
...
function createTeamEmailTemplate(request: RGPDRequest, trackingNumber: string): string {
...
function createUserConfirmationTemplate(request: RGPDRequest, trackingNumber: string): string {
    const requestTypeName = REQUEST_TYPE_NAMES[request.requestType] || request.requestType
    const currentDate = new Date().toLocaleDateString('fr-FR')
